                        ...
                        (projects.map((project, index)=> (
                            <div key={ project.id } className='card p-1 mb-1'>
                                ...
                                <div className='projectList-btn d-flex py-1 px-3 justify-content-end gap-2'>
                                    <button className='btn btn-danger btn-sm' onClick={ () => deleteProject(project.id) }>Delete <i className="bi bi-trash3-fill"></i></button> 
                                    <Link className='btn btn-primary btn-sm' to= { `/editProject/${project.id}` }  style={{ textDecoration: 'none' }}>Edit <i className="bi bi-pencil-fill"></i></Link>  
                                </div>    
                            </div>  )))
                        :
                        (
